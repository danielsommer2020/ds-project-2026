#!/usr/bin/env python3
"""
MovieMatch — Fast Incremental Streaming + Ratings Fetcher (TV Shows)
--------------------------------------------------------------------
Fetches streaming availability AND age ratings (AU/US/GB) per TV show.
...
async def process_show(session, show, semaphore):
    async with semaphore:
        # Pad array to required length
        while len(show) < 13: show.append(None)
        # Index layout:
        # 0-12: app fields
        # 13: streaming_updated (internal)
        # 14: ratings {AU,US,GB} (internal)
        # 15: tmdb_id (internal)
        while len(show) < 16: show.append(None)
        ...
        # Fetch streaming + content_ratings in parallel
        stream_task  = fetch(session, f'{BASE}/tv/{tmdb_id}/watch/providers', {})
        ratings_task = fetch(session, f'{BASE}/tv/{tmdb_id}/content_ratings', {})
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        for i in range(0, len(stale), SAVE_EVERY):
            ...
            results = await asyncio.gather(*[process_show(session, s, semaphore) for s in batch])
            for found in results:
                done += 1
                if found: updated += 1
                else: not_found += 1
            # Save — strip internal fields (keep 0-12 app fields + ratings at 13)
            clean = []
            for s in shows:
                entry = s[:13]           # app fields 0-12
                entry.append(s[14] if len(s)>14 else {})  # ratings at 13
                clean.append(entry)
    ...
    print(f'   Has ratings: {has_ratings:,}/{len(shows):,}')
